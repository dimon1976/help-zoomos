...
(() => {
  ...
  var makeStrategies = (tolerance, index2, clauses, sealed) => [
    ...
    tolerance >= 2 && function bothWildcard(query) {
      query.clauses = clauses.map((clause) => {
        if (clause.presence !== import_lunr.Query.presence.PROHIBITED) {
          wildcard(clause, import_lunr.Query.wildcard.LEADING | import_lunr.Query.wildcard.TRAILING);
        }
        return clause;
      });
    }
  ].filter(Boolean).map(withIndex(index2));
  ...
  function load() {
    return __async(this, null, function* () {
      ...
      const promise = (() => __async(this, null, function* () {
        const [indexData, registry2] = yield Promise.all([
          request(`${config.base}/${config.resources.index}`),
          request(`${config.base}/${config.resources.registry}`)
        ]);
        if (config.resources.language) {
          importScripts(`${config.base}/${config.resources.language}`);
        }
        if (self.language) {
          self.language(import_lunr2.default);
        }
        const index2 = import_lunr2.Index.load(indexData);
        return [index2, registry2];
      }))();
      promise.catch((error) => {
        console.error(error);
      });
  ...
  function request(url) {
    return __async(this, null, function* () {
      const request2 = yield fetch(url);
